...
export function CreateWritableStream<T>(
  ...
  const controller = createWritableStreamDefaultController<T>();
  SetUpWritableStreamDefaultController<T>({
    stream,
    controller,
    startAlgorithm,
    writeAlgorithm,
...
export function IsWritableStream(x: any): x is WritableStream {
...
export function WritableStreamDealWithRejection(
  stream: WritableStream,
  error?: any
) {
  const { state } = stream;
...
export function WritableStreamStartErroring(
  stream: WritableStream,
  reason?: any
) {
...
export class WritableStreamDefaultController<T>
  ...
  queue: { value: { chunk: T } | "close"; size: number }[];
  ...
  writeAlgorithm?: domTypes.WriteAlgorithm<T>;
  [ErrorSteps]: () => void;
  [AbortSteps]: domTypes.AbortAlgorithm;
